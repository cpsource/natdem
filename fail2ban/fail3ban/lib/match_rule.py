...
def match_rule(log_line, jail_regex, compiled_regex=None):
    ...
    if match:
        ...
        return [True, [date_str, jail, host, host_flag, compiled_regex]]
    else:
        # Return flag as False, default 'n/a' values, and the compiled regex
        return [False, ['n/a', 'n/a', 'n/a', 'n/a', compiled_regex]]

def main():
    if len(sys.argv) < 3:
        print("Usage: python3 script_name.py '<log_line>' '<jail_regex>'")
        sys.exit(1)
